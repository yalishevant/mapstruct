...
public abstract class GeneratedType extends ModelElement {
    ...
    private boolean needsImportDeclaration(Type typeToAdd) {
        if ( typeToAdd == null || !typeToAdd.isToBeImported() ) {
            return false;
        }

        if ( !isDeclaredOrArray( typeToAdd ) ) {
            return false;
        }

        String targetPackage = typeToAdd.getPackageName();
        if ( targetPackage == null ) {
            return true;
        }

        if ( JAVA_LANG_PACKAGE.equals( targetPackage ) ) {
        ...
        if ( targetPackage.equals( packageName ) ) {
            if ( typeToAdd.isArrayType() ) {
                Type component = typeToAdd.getComponentType();
                if ( component != null && !isNestedType( component ) ) {
                    return false;
                }
            }
            else if ( !isNestedType( typeToAdd ) ) {
                return false;
            }
        }
    ...
    private boolean isDeclaredOrArray(Type type) {
        if ( type.isArrayType() ) {
            return true;
        }
        ...
        return descriptor.kind() == LangTypeKind.DECLARED;
    }

    private boolean isNestedType(Type type) {
        if ( type == null ) {
            return false;
        }
        ...
        return isNestedElement( elementDescriptor );
    }

    private boolean isNestedElement(TypeElementDescriptor element) {
        if ( element == null ) {
            return false;
        }

        return element.enclosingElement()
            .map( GeneratedType::isNestedEnclosingElement )
            .orElse( false );
    }

    private static boolean isNestedEnclosingElement(ElementDescriptor enclosing) {
        if ( enclosing == null ) {
            return false;
        }
        LangElementKind kind = enclosing.kind();
        if ( kind == LangElementKind.PACKAGE ) {
            return false;
        }
        // Local and anonymous classes should still be treated as nested,
        // so we only shortcut for package-level elements.
        return true;
    }
}
